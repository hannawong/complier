...
class LabelManager:
    ...
    def newLabel(self, scope="_L"):
        if scope not in self.nlabels.keys():
            self.nlabels[scope]=0
        else:
            self.nlabels[scope]+=1
        return str(scope)+str(self.nlabels[scope])
...
class StackIRGen(MiniDecafVisitor):
    ...
    def visitIfStmt(self, ctx:MiniDecafParser.IfStmtContext):
        ...
        endLabel = self.lbl.newLabel("if_end")
        elseLabel = self.lbl.newLabel("if_else")
        if ctx.el is None:
            self._E([Branch("beqz", endLabel)])
            ctx.th.accept(self)
            self._E([Label(endLabel)])
        else:
            self._E([Branch("beqz", elseLabel)])
            ctx.th.accept(self)
            self._E([Branch("br", endLabel)])
